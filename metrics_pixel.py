...
def metrics_name():
    ...
    resroot_list =  os.listdir(args.res_root)
    
    if not args.ref_root:
        args.ref_root=args.res_root
    
    dirs= []
    for i in resroot_list:
        if os.path.isdir(os.path.join(args.res_root,i)):
            dirs.append(i)
    ...
    if os.path.basename(args.res_root):
        ...
        basename = os.path.basename(args.res_root)
        dir_root = os.path.dirname(args.res_root)
    else:
        basename = os.path.basename(os.path.dirname(args.res_root))
        dir_root = os.path.dirname(os.path.dirname(args.res_root))
    
    Record_eachfile = os.path.join(dir_root,'%s_PSNR_perfile.txt'%(basename))
    Record_eachvideo = os.path.join(dir_root,'%s_PSNR.txt'%(basename))

    if os.path.exists(Record_eachfile):
        os.system('rm %s'%Record_eachfile)
    # if os.path.exists(Record_eachvideo):
    #     os.system('rm %s'%Record_eachvideo)      
    
    if multiple_video:
        for path in dirs:

            ref_path = os.path.join(args.ref_root,path,'target')
            res_path = os.path.join(args.res_root,path)
            gt_files = natsorted(os.listdir(ref_path))
            res_files = os.listdir(res_path)
            res_files = natsorted([i for i in res_files if i.endswith('restored.png')])
            ...
            for i in range(len(res_files)):
                ...
                write_txt(Record_eachfile,'%s psnr/lpips: %.2f %.4f'%(res_files[i],per_psnr, per_lpips))
            # line = 'result for video %s ssim:%.4f'%(path, ssim_video / cnt_video)
            # write_txt(record_file, line)
            line = 'result for video %s \t psnr:%.2f \t ssim:%.4f \t lpips: %.3f'%(path, psnr_video / cnt_video, ssim_video/cnt_video, lpips_video/cnt_video)
            write_txt(Record_eachvideo, line)
            t_ssim += ssim_video
            t_psnr += psnr_video
            t_lpips += lpips_video
            cnt += cnt_video
