...
def define_BDCN(config):
    net = bdcn.BDCN()
    net = init_net(net,gpu_ids=config['gpu'])
    return net
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='xavier', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    # if netF == 'global_pool':
    #     net = PoolingF()
    if netF == 'reshape':
        net = ReshapeF()
