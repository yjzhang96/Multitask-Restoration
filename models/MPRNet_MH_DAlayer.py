...
class ResBlock(nn.Module):
    def __init__(self, in_feat, out_feat, kernel_size, bias, act):
        ...
        modules_body.append(conv(out_feat, out_feat, kernel_size, bias=bias))
        self.body = nn.Sequential(*modules_body)
...
class Head(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, head_num, bias, act):
        super(Head, self).__init__()
        self.head = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, padding=1),
                ResBlock(out_channel, out_channel, kernel_size, bias, act),
                ResBlock(out_channel, out_channel, kernel_size, bias, act)
    ...
    def forward(self, x, index):
        # if len(index) > 1:
        #     index = index[0]
        return self.head[index](x)


##########################################################################
class MPRNet_MH_DAlayer(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, degrade_num=4, use_type_emb=False,
        ...
        super(MPRNet_MH_DAlayer, self).__init__()
        ...
        self.head_conv1 = Head(in_c, n_feat, kernel_size, head_num=degrade_num, bias=bias, act=act)
        self.head_conv2 = Head(in_c, n_feat, kernel_size, head_num=degrade_num, bias=bias, act=act)
        self.head_conv3 = Head(in_c, n_feat, kernel_size, head_num=degrade_num, bias=bias, act=act)
    ...
    def forward(self, x3_img, index):
        ...
        x1ltop = self.shallow_feat1(self.head_conv1(x1ltop_img, index[0]), index_emb)
        x1rtop = self.shallow_feat1(self.head_conv1(x1rtop_img, index[0]), index_emb)
        x1lbot = self.shallow_feat1(self.head_conv1(x1lbot_img, index[0]), index_emb)
        x1rbot = self.shallow_feat1(self.head_conv1(x1rbot_img, index[0]), index_emb)
        ...
        ##-------------------------------------------
        ## Compute Shallow Features
        x2top  = self.shallow_feat2(self.head_conv2(x2top_img, index[0]), index_emb)
        x2bot  = self.shallow_feat2(self.head_conv2(x2bot_img, index[0]), index_emb)
        ...
        ##-------------------------------------------
        ## Compute Shallow Features
        x3     = self.shallow_feat3(self.head_conv3(x3_img, index[0]), index_emb)
